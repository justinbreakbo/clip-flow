...
Android 10+ 对后台剪贴板访问有系统级限制。Clip Flow 不把“后台完全静默写入剪贴板”作为默认承诺，而是按三层体验推进：

1. 通知确认写入：作为默认体验，后台收到远端内容后通知用户，点击通知后直接复制到 Android 剪贴板。
2. 无障碍增强模式：作为可选实验功能，用于改善输入场景提示和一键粘贴，不默认开启。
3. 极简输入法模式：作为长期高级模块，通过 `Clip Flow Keyboard` 提供候选栏、一键粘贴和最近剪贴板。

详细分析见 [docs/android-background-sync-strategy.md](./docs/android-background-sync-strategy.md)。
...
- 设置页可配置 relay URL、Secret、设备名、是否自动接收 offer。
- App 打开后默认自动启动前台同步服务；界面提供 `Restart Sync` 用于重启并加载新配置。
- 发送输入框文本。
- 发送 Android 剪贴板文本。
...
- App 后台收到远端文本时会保存为 pending clipboard，并通过高优先级通知提示用户；点击通知后直接复制到 Android 剪贴板。
- App 启动时会检查 `Clip Flow Clipboard Alerts` 通知渠道；如果通知总权限未开启或渠道重要性不足，会引导用户跳转系统通知设置。
...
- 后台收到远端内容后的通知确认写入继续完善：待处理列表、敏感内容确认页、持久化日志。
- 可选无障碍模式，用于更强的自动同步。
- 极简输入法模式，用于更稳定的输入场景剪贴板体验。
- 安全模式和手动模式。
- 更完整的通知栏控制。
- 快捷设置磁贴。
- 系统权限引导。
